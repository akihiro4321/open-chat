...
void test('RAG検索helpは設定なしでも使い方を表示して正常終了する', async () => {
  const result = await execFileAsync(
    process.execPath,
    ['--import', 'tsx', 'src/rag-cli.ts', 'search', '--help'],
    {
      cwd: process.cwd(),
      env: environmentWithoutOpenAISettings(),
    },
  ...
  assert.match(result.stdout, /rag:search/);
  assert.match(result.stdout, /--retrieval-mode/);
  assert.equal(result.stderr, '');
});

void test('RAG検索の質問が未指定なら設定読み込み前にエラーを表示する', async () => {
  await assert.rejects(
    execFileAsync(process.execPath, ['--import', 'tsx', 'src/rag-cli.ts', 'search'], {
      cwd: process.cwd(),
      env: environmentWithoutOpenAISettings(),
    }),
    (error: unknown) => {
      ...
      const stderr = String(error.stderr);
      return (
        error.code === 1 && stderr.includes('--question は必須です。') && !stderr.includes('OPENAI')
      );
    },
  );
});
